...
std::tuple<std::vector<at::Tensor>, std::vector<py::object>> split_bgrad_quantize(
  ...
  auto fallback = [&]() -> std::tuple<std::vector<at::Tensor>, std::vector<py::object>> {
    std::vector<int64_t> split_sections_i64(split_sections.begin(), split_sections.end());
    auto split_tensors = tensor.contiguous().split_with_sizes(split_sections_i64, 0);
  ...
  std::vector<Float8BlockQuantizer *> blockwise_quantizers;
  blockwise_quantizers.reserve(num_splits);
  for (auto &quantizer : quantizer_cpp_list) {
    blockwise_quantizers.push_back(static_cast<Float8BlockQuantizer *>(quantizer.get()));
  }
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    const int64_t bias_size = static_cast<int64_t>(split_shapes[i].back());
  ...
  return {std::move(grad_bias_torch_list), std::move(output_py_list)};
}
