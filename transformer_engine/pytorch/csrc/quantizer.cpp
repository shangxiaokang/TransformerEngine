...
std::pair<TensorWrapper, py::object> Float8Quantizer::create_tensor(
  ...
  if (internal) {
    ...
    PyObject* result = PyObject_Call(reinterpret_cast<PyObject*>(Float8TensorBasePythonClass),
                                     args.ptr(), kwargs.ptr());
    if (result == nullptr) {
      PyErr_Print();
    }
    NVTE_CHECK(result != nullptr, "Failed to create Float8TensorBase instance");
...
std::pair<TensorWrapper, py::object> Float8BlockQuantizer::create_tensor(
  ...
  auto rowwise_data_py = py_cast(data_rowwise, rowwise_usage);
  auto rowwise_scale_inv_py = py_cast(scale_inv_rowwise, rowwise_usage);
  auto columnwise_data_py = py_cast(data_colwise, columnwise_usage);
  auto columnwise_scale_inv_py = py_cast(scale_inv_colwise, columnwise_usage);
  ...
  if (internal) {
    ...
    kwargs["data_format"] = py::cast(data_format);

    PyObject* result = PyObject_Call(
        reinterpret_cast<PyObject*>(Float8BlockwiseQTensorBasePythonClass), args.ptr(), kwargs.ptr());
    if (result == nullptr) {
      PyErr_Print();
    }
    NVTE_CHECK(result != nullptr, "Failed to create Float8BlockwiseQTensorBase instance");
    ...
    PyObject* result = PyObject_Call(
        reinterpret_cast<PyObject*>(Float8BlockwiseQTensorPythonClass), args.ptr(), kwargs.ptr());
    if (result == nullptr) {
      PyErr_Print();
    }
...
std::pair<TensorWrapper, py::object> MXFP8Quantizer::create_tensor(const std::vector<size_t>& shape,
  ...
  if (internal) {
    ...
    PyObject* result = PyObject_Call(reinterpret_cast<PyObject*>(MXFP8TensorBasePythonClass),
                                     args.ptr(), kwargs.ptr());
    if (result == nullptr) {
      PyErr_Print();
    }
    NVTE_CHECK(result != nullptr, "Failed to create MXFP8TensorBase instance");
    out_py = py::reinterpret_steal<py::object>(result);
  } else {
    py::dict kwargs;
    py::tuple args(0);
...
std::pair<TensorWrapper, py::object> NVFP4Quantizer::create_tensor(const std::vector<size_t>& shape,
  ...
  if (internal) {
    ...
    PyObject* result = PyObject_Call(reinterpret_cast<PyObject*>(NVFP4TensorBasePythonClass),
                                     args.ptr(), kwargs.ptr());
    if (result == nullptr) {
      PyErr_Print();
    }
    NVTE_CHECK(result != nullptr, "Failed to create NVFP4TensorBase instance");
    out_py = py::reinterpret_steal<py::object>(result);
  } else {
    py::dict kwargs;
    py::tuple args(0);
