...
class _Fp8Padding(torch.autograd.Function):
    ...
    def forward(
        ...
        if isinstance(inp, Float8BlockwiseQTensorBase):
            if inp._is_2D_scaled or inp._rowwise_data is None or inp._rowwise_scale_inv is None:
                raise NotImplementedError("FP8 padding only supports 1D rowwise FP8 tensors")
            out_data = torch.empty(
                [total_row, in_features], dtype=inp._rowwise_data.dtype, device=inp.device
            )
            tex.fused_multi_row_padding(
                inp._rowwise_data.view(-1, in_features), out_data, m_splits, padded_m_splits
            )
            if inp._is_gemm_ready_format():
                scale_inv = inp._rowwise_scale_inv.transpose(-2, -1).contiguous()
                out_scale_inv = torch.empty(
                    [total_row, scale_inv.shape[-1]], dtype=scale_inv.dtype, device=scale_inv.device
                )
                tex.fused_multi_row_padding(
                ...
                out_scale_inv = out_scale_inv.transpose(-2, -1).contiguous()
            else:
            ...
            out = Float8BlockwiseQTensor(
                ...
                rowwise_scale_inv=out_scale_inv,
            ...
            if isinstance(grad_output, Float8BlockwiseQTensorBase):
                if (
                    ...
                    raise NotImplementedError(
                        "FP8 padding backward only supports 1D rowwise FP8 tensors"
                    )
